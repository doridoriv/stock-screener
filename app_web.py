...
# 데이터시트 가로 컴팩트 가독성 보장 줄바꿈 제어 스타일시트 주입
st.markdown("""
<style>
...
st.markdown("yfinance 통합 실시간 체제 전환 완료. 이제 국장 장중 시간대에도 에러 없이 최적화된 주식 쇼핑이 가능합니다.")

st.sidebar.header("🔍 스크리너 설정")
market = st.sidebar.selectbox("시장 선택", ["미국", "한국(코스피)", "한국(코스닥)"])
top_n = st.sidebar.slider("분석 대상 종목 수", 5, 50, 20)
...
if btn_stop:
    if st.session_state.stop_event is not None:
        ...
        st.warning("⚠️ 백엔드 워커 스레드에 정지 명령 처리를 요청했습니다.")
...
if btn_search:
    ...
    while t_worker.is_alive() or not app_queue.empty():
        try:
            ...
            if packet.get("type") == "data":
                ...
                status_box.info(f"⚡ yfinance로부터 실시간 파싱 진행중... 현재 {len(collected_records)}개 완료")
            elif packet.get("type") == "info":
                st.info(packet.get("text"))
            elif packet.get("type") == "error":
                st.error(f"❌ 분석 오류: {packet.get('text')}")
    ...
    status_box.success(f"🎉 {market} 실시간 분석 완료!")
...
if btn_load:
    ...
    if target_path and os.path.exists(target_path):
        try:
            ...
            st.success(f"📂 로컬 동기화 백업 캐시 로드 완료 ({len(st.session_state.data)}개 항목)")
        except Exception as e:
            st.error(f"캐시 파일 로드 실패: {e}")
    else:
        st.error("📂 가용한 로컬 캐시 데이터 저장본이 없습니다. 실시간 검색을 구동해 주세요.")
...
def style_screener_dataframe(df, m_ctx):
    def color_per(v):
        try:
            ...
            if val <= 5: return "color: #0D47A1; font-weight: bold;"
            elif val <= 10: return "color: #1976D2; font-weight: bold;"
            elif val <= 15: return "color: #4FC3F7; font-weight: bold;"
            elif val <= 20: return "color: #EF6C00; font-weight: bold;"
    ...
    def color_pbr(v):
        try:
            val = float(v)
            if pd.isna(val) or val <= 0: return "color: #757575;"
...
if st.session_state.data:
    ...
    # 상단 요약 대시보드 UI 영역 (04 스크리너해석 v2 요구사항 완벽 준수)
    st.markdown("---")
    st.markdown(f"""
    <div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;'>
        <h3 style='margin: 0;'>📊 종합 스크리닝 계측 현황</h3>
        <span style='background-color: #E5E7EB; padding: 6px 14px; border-radius: 20px; font-weight: bold; font-size: 14px; color: #1F2937;'>📅 데이터 기준일 {date_val}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("### 🔍 가장 먼저 차트를 열어볼 후보 (A등급 탑픽)")
    ...
    if not a_candidates.empty:
        ...
        st.info("💡 현재 수집 풀에 가점 기준 합격점(75점 이상)을 득한 A등급 종목이 없습니다. 아래 상세 데이터 시트에서 고점의 B등급 우량 기업을 우선적으로 검토하세요.")

    st.markdown("### 📋 상세 저평가 분석 데이터 시트")
