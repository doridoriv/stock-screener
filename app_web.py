...
import analyzer
from config import APP_TITLE, CACHE_DIR, COL_INFOS
...
# ==============================================================================
# config.py 설정 기반 링크 및 컬럼 너비 맵 생성
# ==============================================================================
id_to_web_name = {
    "rank": "순위", "symbol": "티커", "name": "종목명", "data_date": "기준일",
    "market_cap": "시가총액(억)", "price": "현재가", "peak": "최고점",
    ...
    while True:
        try:
            ...
            if m_type == "progress":
                ...
                # [수정] 실시간 수신 시 시가총액 순위(rank) 기준으로 항상 오름차순 정렬하여 출력 유지
...
if st.session_state.data:
    ...
    # [수정] 최종 결과 테이블도 불러오기나 정렬 유실 시 시가총액 순위(rank) 기준으로 재정렬 보장
